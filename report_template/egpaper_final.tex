...
According to Goodfellow \cite{goodfellow}, Dong \cite{momentum}, Chen \cite{frank} and Rinaldi \cite{rinaldi}, we use the \textit{Projected Gradient Descent}, the \textit{Fast Gradient Sign}, the \textit{Momentum Iterative Fast Gradient Sign} and the \textit{Frank-Wolfe-white} methods. 
...
It is not iterative and updates only once the input value $x_{ori}$. 
...
The Frank Wolf method is an iterative first-order optimization algorithm. It is widely used in data science since it is a projection-free algorithm and has a smaller cost per iteration w.r.t. projection methods.
