...
In line 1, $\gamma$ is fixed in order to respect the constrain $\Vert x - x_{\text{ori}} \Vert_{\infty} \le \epsilon$ and the momentum term can be found in line 4 with a decay factor $\beta$. An adversarial example $x_{T}^{\ast}$ is perturbed in the direction of $g_t$ with a step size of $\gamma$. Moreover at each iteration the current gradient is normalized with its $L_1$ norm (line 4) in order to correct the gradients scale.
...
The general idea of this method is to call a \textit{Linear Minimization Oracle} (LMO): starting from a feasible solution, at each iteration we define a descent direction as the solution of:
\begin{equation*}
    \min_{x \in C}\nabla f(x_k)^T (x-x_k)\,.
\end{equation*}
